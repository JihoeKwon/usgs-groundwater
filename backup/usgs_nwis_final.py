...
class USGSGroundwaterAPI:
    ...
    def get_recent_groundwater_data(self, state_code="CA", days=7, max_sites=50):
        ...
        try:
            ...
            print(f"\nProcessing data from {len(value)} sites...")
            ...
            for i, site_data in enumerate(value[:max_sites], 1):
                try:
                    ...
                    if i % 10 == 0:
                        print(f"  {i}/{min(len(value), max_sites)} processed...")
            ...
            print(f"\nTotal {len(sites_df)} sites")
            print(f"Total {len(data_df)} data points")

            return {'sites': sites_df, 'data': data_df}

        except requests.exceptions.Timeout:
            print("Timeout occurred. Try reducing the period or try again later.")
            return {'sites': pd.DataFrame(), 'data': pd.DataFrame()}
    ...
    def get_site_history(self, site_no, start_date=None, end_date=None, days=365):
        """
        Query historical data for a specific monitoring station
        ...
        end_date : str
            End date (YYYY-MM-DD)
        days : int
            Number of recent days (if start_date/end_date not provided)
        ...
        print(f"\nQuerying data for site {site_no}...")
...
def main():
    ...
    print("=" * 80)
    print("USGS NWIS API - California Groundwater Data Collection")
    print("=" * 80)

    client = USGSGroundwaterAPI()

    # 1. Query California groundwater stations with data from the last 30 days
    print("\n[1] Query California groundwater stations and recent data")
    print("-" * 80)
